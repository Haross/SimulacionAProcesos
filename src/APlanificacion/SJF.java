...
public class SJF extends Thread{
    int t = 0; //almacena todos  el tiempo en cpu
    int te = 0;
   ...
   public SJF(TableView tableV,TextArea txtTe,TextArea  txtTr,TextArea txtP){
       this.tableV = tableV;
       this.txtP = txtP;
       this.txtTe = txtTe;
       this.txtTr = txtTr;
   ...
   private String getTiempoEspera(int tiempoL){
    ...
    @Override
    public void run(){
        System.out.println("corriendo SJF");
        do{
            if(!colaEspera.isEmpty()){   
                ...
                sumPe += penalizacion;
                sumTe += te;
                setTabla(split[1],split[0],te+"",trespuesta+"",penalizacion+"",numProceso+"");
                t = t+ thick;
                cpu.add(aux );
